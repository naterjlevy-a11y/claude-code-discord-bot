...
# Force stdout to UTF-8 / line-buffered so bot.log shows print() output as it happens
# (block buffering otherwise hides logs until shutdown, making mid-run debugging hard).
try:
...
# Auto / "bypass permissions" mode — launch terminal Claude sessions with
# --dangerously-skip-permissions so tool actions run without a Discord approval tap
# (the point of headless/remote use). On by default; set CC_SKIP_PERMISSIONS=0 in .env
# to go back to per-tool approval.
SKIP_PERMISSIONS = os.environ.get("CC_SKIP_PERMISSIONS", "1") != "0"
CLAUDE_BIN = "claude --dangerously-skip-permissions" if SKIP_PERMISSIONS else "claude"
...
HELP_TEXT = (
    ...
    f"`{PREFIX} close [name]` — detach, **kill the tmux session**, delete the channel\n"
    f"`{PREFIX} cleanup` — sweep orphan tmux sessions whose claude already exited\n"
    f"`{PREFIX} sweep` — delete orphan hex-id Discord channels (no live claude)\n\n"
...
attached_pids: Dict[int, int] = {}  # channel_id → live claude PID (in-memory only)
...
_session_shell_pid: Dict[int, int] = {}  # claude pid → parent pid (unused on macOS; tmux owns teardown)
# claude PID → the tmux session name hosting it. Recorded at spawn; target_for_pid()
# can re-derive it at any time, but the map is faster and lets the close/outbox
# watchers map a tmux SESSION NAME (what the .sh helpers write) back to a channel.
_pid_session_name: Dict[int, str] = {}
# Drop-a-file close signal: the `/cc-close` slash command (via cc_close.sh) writes
# close-markers/<tmux-session-name>; _close_marker_watcher acts on it within ~2s.
CLOSE_MARKER_DIR = Path(__file__).parent / "close-markers"
# Outbox: `/cc-send <file>` (via cc_send.sh) copies a file to outbox/<session>__<name>;
# _outbox_watcher uploads it to that session's Discord channel, then deletes it.
OUTBOX_DIR = Path(__file__).parent / "outbox"


def _resolve_target(pid: int) -> Optional[str]:
    """claude PID → tmux session name (its drive target). Uses the recorded map first,
    falls back to walking the process tree via tmux/ps. Caches what it finds."""
    if not pid:
        return None
    name = _pid_session_name.get(pid)
    if name and mac_terminal.has_session(name):
        return name
    target = mac_terminal.target_for_pid(pid)
    if target:
        _pid_session_name[pid] = target
    return target


def _is_bot_managed(pid: int) -> bool:
    """True ONLY for claude sessions the bot launched into a `cc-` tmux session.

    This is the key macOS difference from the Windows original: on Windows every
    claude.exe was, by definition, a bot terminal. On macOS the user also runs `claude`
    for unrelated things (the local CLI, IDE probes, the bot's own SDK subprocesses), so
    the auto-adoption / reconciler paths must NOT grab those — otherwise every random
    claude turns into a hex-named Discord channel that mirrors unrelated activity. A
    session is "ours" iff its PID resolves to a tmux session named `cc-…` (which only the
    bot's launcher creates)."""
    t = _resolve_target(pid)
    return bool(t and t.startswith(mac_terminal.SESSION_PREFIX))


def _channel_for_session(session_name: str):
    """Map a tmux SESSION NAME back to (channel_id, claude_pid). The close/outbox .sh
    helpers identify themselves by session name (`tmux display-message -p '#S'`), so the
    watchers need this reverse lookup. Checks in-memory attachments first, then the DB."""
    for cid, pid in list(attached_pids.items()):
        if _pid_session_name.get(pid) == session_name or _resolve_target(pid) == session_name:
            return cid, pid
    for cid, pid in sessions.all_attached():
        if pid and _resolve_target(pid) == session_name:
            return cid, pid
    return None, None
...
def _list_claude_pids() -> set:
    """All running `claude` PIDs. `pgrep -x claude` matches the native binary by name
    (so it catches a session the instant it spawns, before it writes its registry
    JSON). Falls back to the session registry if pgrep is unavailable."""
    try:
        proc = subprocess.run(
            ["pgrep", "-x", "claude"], capture_output=True, text=True, timeout=10,
        )
        ...
        pass
    return {c.pid for c in list_running()}


def _spawn_session_window(inner_cmd: str, cwd: Optional[str], title: Optional[str] = None) -> str:
    """Launch a Claude session inside a fresh, detached tmux session and return its
    NAME (`cc-<sanitized-title>`, with a random suffix on collision).

    This is the macOS analogue of the Windows "open a Windows Terminal tab" launcher.
    The tmux server lives independently of the bot, so the session survives a bot
    restart / crash — which is what the reboot-restore + auto-resume machinery relies
    on. The bot still tracks the session by its claude PID; `target_for_pid` /
    `_resolve_target` map that PID back to this tmux name when driving it.
    """
    use_cwd = cwd if (cwd and cwd not in ("?", "") and Path(cwd).is_dir()) else None
    name = mac_terminal.unique_session_name(title or "claude")
    if not mac_terminal.new_session(name, use_cwd, inner_cmd):
        raise RuntimeError(f"tmux new-session failed for {name!r}")
    return name
...
async def _drive_resume_startup(pid: int, max_wait: float = 90.0) -> str:
    ...
    while time.time() < deadline:
        ...
        if (not did_trust and "enter to confirm" in screen
            ...
            continue
        # Bypass-permissions warning (from --dangerously-skip-permissions). Appears AFTER
        # trust. Its default highlight is "1. No, exit" — pressing Enter here would KILL
        # the session, so we must move Down to "2. Yes, I accept" then confirm. Without
        # this, every auto-mode launch dies on this screen (no session JSON ever appears).
        if (not did_bypass and "bypass permissions mode" in screen
                and "yes, i accept" in screen and "enter to confirm" in screen):
            await _run_console_helper(pid, "down,enter", mode="keys")  # select "Yes, I accept"
            did_bypass = True
            await asyncio.sleep(1.5)
            continue
    ...
    return f"trust={did_trust} bypass={did_bypass} full_resume={did_resume}"
...
async def cmd_resume_spawn(channel, user_id: int, s):
    ...
    try:
        sess_name = _spawn_session_window(f"{CLAUDE_BIN} --resume {s.session_id}", cwd,
    ...
    if not new_pid:
        await channel.send("⚠️ tmux session started but no claude process appeared.")
        return

    # Record the PID → tmux-session mapping so we can drive it without re-walking ps.
    _pid_session_name[new_pid] = sess_name
...
async def cmd_usage(channel):
    ...
    lines.append(
        ...
        f"context: **{pct:.1f}%** · {msgs} msgs ({prompts} prompts)"
    )
...
async def cmd_launch(channel, user_id: int, args: str):
    ...
    try:
        sess_name = _spawn_session_window(CLAUDE_BIN, cwd, title=name)
    except Exception as e:
        await channel.send(f"⚠️ Couldn't launch: `{type(e).__name__}: {e}`")
        return

    # Wait for a new claude process to appear via pgrep (faster than waiting for the
    # session JSON, which doesn't exist until the trust prompt is accepted).
    new_pid: Optional[int] = None
    deadline = time.time() + 30
    while time.time() < deadline:
        await asyncio.sleep(1)
    ...
    if not new_pid:
        await channel.send(
            "⚠️ Launched the tmux session but no new claude process appeared. "
            f"Check it with `tmux attach -t {sess_name}`."
        )
        return
    ...
    if "(failed)" in rename_helper_out:
        await channel.send(
...
def _get_parent_pid_sync(pid: int) -> Optional[int]:
    """Best-effort parent-PID lookup via `ps -o ppid=`. None for dead pids / failure.
    (Largely vestigial on macOS — tmux owns terminal teardown — but kept for the few
    callers that still record a parent pid.)"""
...
def _kill_tree(pid: int) -> bool:
    """End claude `pid` by killing the tmux session that hosts it (which tears down the
    whole pane), falling back to SIGKILL on the pid itself. Returns True if it's dead."""
    target = _resolve_target(pid)
    if target:
        mac_terminal.kill_session(target)
        _pid_session_name.pop(pid, None)
    if pid_alive(pid):
        try:
            os.kill(pid, signal.SIGKILL)
        except OSError:
            pass
    time.sleep(0.3)  # give the OS a moment to actually tear down
    return not pid_alive(pid)


async def _close_terminal_for_pid(pid: int) -> str:
    """Kill the tmux session hosting claude `pid`. Returns user-facing status."""
    target = await asyncio.to_thread(_resolve_target, pid)
    if not target:
        if not pid_alive(pid):
            return "terminal already exited"
        return f"no tmux session found for pid {pid} — try `!cc cleanup`"
    ok = await asyncio.to_thread(mac_terminal.kill_session, target)
    _pid_session_name.pop(pid, None)
    return f"closed tmux session `{target}`" if ok else f"tried to kill `{target}` but it's still alive"


async def _close_session_cleanly(ch_id, session_name: Optional[str], claude_pid: int, reason: str) -> None:
    """Tear a session down cleanly and permanently (no auto-resume, no re-channel).

    `ch_id` may be None (an orphaned session with no channel) — we still kill the tmux
    session so a close always closes the terminal. Order: mark CLOSING (so the reconciler
    can't re-create a channel for it mid-teardown) + UNTRACK first; kill the tmux session
    and WAIT for claude to actually die (otherwise the reconciler sees a 'live session
    with no channel' and re-adds one); finally delete the Discord channel.
    """
    if claude_pid:
    ...
    try:
        ...
        # Kill the tmux session (ends claude + everything in the pane).
        if not session_name and claude_pid:
            session_name = await asyncio.to_thread(_resolve_target, claude_pid)
        if session_name and await asyncio.to_thread(mac_terminal.has_session, session_name):
            await asyncio.to_thread(mac_terminal.kill_session, session_name)
        if claude_pid:
            _pid_session_name.pop(claude_pid, None)
            for _ in range(20):  # wait up to ~4s for claude to actually exit
        ...
        print(f"  cleanly closed (ch={ch_id}, {reason}; claude {claude_pid}, tmux {session_name})")
    finally:
        if claude_pid:
...
async def _close_marker_watcher(interval: float = 2.0):
    ...
    The slash helper (cc_close.sh) identifies itself by tmux SESSION NAME — it writes
    close-markers/<session-name> (`tmux display-message -p '#S'`). We map the name back
    to its channel/pid and close. /cc-close must ALWAYS close: if the tmux session is
    live we kill it even when no Discord channel is bound. Stale markers from before this
    process started are cleared on boot so they can't mis-fire.
    """
    try:
    ...
    while True:
        try:
            ...
            for mf in list(CLOSE_MARKER_DIR.glob("*")):
                session_name = mf.name
                ch_id, claude_pid = _channel_for_session(session_name)
                live = await asyncio.to_thread(mac_terminal.has_session, session_name)
                if ch_id is not None or live:
                    await _close_session_cleanly(ch_id, session_name, claude_pid or 0, "cc-close command")
                else:
                    print(f"  cc-close marker for dead/unknown session {session_name!r} — ignoring")
                try:
                    mf.unlink()
                except OSError:
                    pass
...
async def _outbox_watcher(interval: float = 3.0):
    ...
    Filenames are `<tmux-session-name>__<original_name>` (cc_send.sh identifies the
    session by `tmux display-message -p '#S'`). Upload to that session's Discord channel,
    then delete the file. A file whose session never gets a channel (truly-gone session)
    is dropped after a ~30s grace so the outbox can't pile up.
    """
    try:
    ...
    while True:
        try:
            ...
            for f in list(OUTBOX_DIR.glob("*__*")):
                session_name, _, orig = f.name.partition("__")
                if not session_name:
                    try:
                        f.unlink()
                    except OSError:
                        pass
                    continue
                ch_id, _pid = _channel_for_session(session_name)
                ...
                if chan is None:
                    ...
                    if n >= 10:  # ~30s
                        print(f"  outbox: no channel for session {session_name!r} after 30s — dropping {orig}")
                        try:
                            f.unlink()
                        except OSError:
                            pass
...
async def cmd_cleanup(channel):
    """Sweep up orphan tmux sessions (cc-*) whose `claude` has already exited."""
    cc_sessions = await asyncio.to_thread(mac_terminal.list_cc_sessions)
    if not cc_sessions:
        await channel.send("🧹 No `cc-` tmux sessions found.")
        return

    # tmux sessions that currently host a live claude — healthy, must NOT be killed.
    live_targets = set()
    for c in list_running():
        t = await asyncio.to_thread(_resolve_target, c.pid)
        if t:
            live_targets.add(t)

    killed: list[str] = []
    skipped: list[str] = []
    for name in cc_sessions:
        if name in live_targets:
            skipped.append(name)
            continue
        if await asyncio.to_thread(mac_terminal.kill_session, name):
            killed.append(name)
    ...
    if killed:
        s = "s" if len(killed) > 1 else ""
        lines.append(
            f"🧹 Closed **{len(killed)}** orphan tmux session{s}: "
            + ", ".join(f"`{n}`" for n in killed)
        )
    if skipped:
        s = "s" if len(skipped) > 1 else ""
        lines.append(f"Left **{len(skipped)}** session{s} alone (live claude inside).")
    if not killed and not skipped:
        lines.append("Nothing to clean up.")
    await channel.send("\n".join(lines))


# Screen-stability tuning for `send` mode — mirrors console_helper.py's constants.
_SEND_STABLE_SECONDS = 1.5    # screen must be unchanged this long after a real change
_SEND_MAX_WAIT = 60.0         # absolute cap on a send poll
_SEND_NO_CHANGE_TIMEOUT = 8.0  # bail if the screen never reacts at all
_SEND_POLL = 0.25


async def _run_console_helper(pid: int, prompt: str, mode: str) -> str:
    """Drive the tmux session hosting claude `pid`. The macOS replacement for the Win32
    console_helper subprocess — same modes, same return-string contracts so every caller
    in this file works unchanged:

      look  → capture the screen and return it
      type  → type text + Enter (no screen poll); returns "typed"
      send  → type text, then poll the screen until it's been stable for ~1.5s (used for
              slash commands that render a static panel); returns the final screen
      keys  → send a comma-separated key sequence; returns "keys-sent: …"
      esc   → send Escape; returns "esc-sent"
      enter → send Enter; returns "enter-sent"

    All blocking tmux calls run in threads so the event loop never stalls. If the pid has
    no live tmux session, returns a string containing "failed" (the callers that check for
    errors look for that token).
    """
    target = await asyncio.to_thread(_resolve_target, pid)
    if not target:
        return f"⚠️ no tmux session for pid {pid} (failed)\n"

    if mode == "look":
        return await asyncio.to_thread(mac_terminal.capture, target)

    if mode == "esc":
        await asyncio.to_thread(mac_terminal.send_esc, target)
        return "esc-sent\n"

    if mode == "enter":
        await asyncio.to_thread(mac_terminal.send_enter, target)
        return "enter-sent\n"

    if mode == "keys":
        try:
            await asyncio.to_thread(mac_terminal.send_key_sequence, target, prompt)
        except ValueError as e:
            return f"keys failed: {e}\n"
        return f"keys-sent: {prompt}\n"

    if mode == "type":
        await asyncio.to_thread(mac_terminal.send_text, target, prompt, True)
        return "typed\n"

    if mode == "send":
        # Snapshot BEFORE typing so we can tell whether the TUI has actually reacted yet,
        # then poll until the screen settles (or we hit a timeout).
        baseline = await asyncio.to_thread(mac_terminal.capture, target)
        await asyncio.to_thread(mac_terminal.send_text, target, prompt, True)
        type_done_at = time.time()
        last = baseline
        last_change = type_done_at
        saw_real_change = False
        deadline = type_done_at + _SEND_MAX_WAIT
        while time.time() < deadline:
            current = await asyncio.to_thread(mac_terminal.capture, target)
            if current != last:
                last = current
                last_change = time.time()
                if current != baseline:
                    saw_real_change = True
            elif saw_real_change and (time.time() - last_change) >= _SEND_STABLE_SECONDS:
                break
            elif (not saw_real_change) and (time.time() - type_done_at) >= _SEND_NO_CHANGE_TIMEOUT:
                break
            await asyncio.sleep(_SEND_POLL)
        return last

    # Unknown mode — just snapshot.
    return await asyncio.to_thread(mac_terminal.capture, target)
...
class StopTurnView(discord.ui.View):
    """A 🛑 Stop button posted while an SDK turn runs. Tapping it cancels the
    in-flight turn for this channel (same as `!cc cancel`, but one tap from the phone)."""

    def __init__(self, channel_id: int):
        super().__init__(timeout=1800)  # 30 min — auto-clears if a turn runs absurdly long
        self.channel_id = channel_id

    @discord.ui.button(label="Stop", style=discord.ButtonStyle.danger, emoji="🛑")
    async def btn_stop(self, interaction: discord.Interaction, _button: discord.ui.Button):
        if not _is_authorised(interaction.user.id, interaction.channel_id):
            await interaction.response.send_message("Unauthorized.", ephemeral=True)
            return
        task = active_turns.get(self.channel_id)
        if task is None or task.done():
...
async def cmd_ask(channel, channel_id, user_id, prompt: str):
    ...
    if channel_id in active_turns and not active_turns[channel_id].done():
        await channel.send("A turn is already running in this channel — tap 🛑 **Stop** or `cancel` it first.")
        return
    ...
    # Post a Stop button so the turn can be killed with one tap from the phone.
    stop_view = StopTurnView(channel_id)
    try:
        stop_view.message = await channel.send("⚙️ _Working…_", view=stop_view)
    except discord.HTTPException:
        stop_view.message = None
    ...
    try:
        ...
        # Disable + tidy the Stop button now the turn is over.
        for child in stop_view.children:
            child.disabled = True
        stop_view.stop()
        if stop_view.message is not None:
            try:
                await stop_view.message.delete()
            except discord.HTTPException:
                pass
    ...
    while True:
        try:
            ...
            for ch_id, pid in tracked.items():
                if pid_alive(pid):
                    # On macOS tmux owns terminal teardown, so there's no separate "shell
                    # stays alive after /exit" signal to track — kept as 0 so the legacy
                    # orphan-shell branches below stay no-ops.
                    if ch_id not in CONTROL_CHANNELS and pid not in _session_shell_pid:
                        _session_shell_pid[pid] = 0
        ...
        try:
            ...
            if guild is not None:
                for c in running:
                    ...
                    if not _is_bot_managed(c.pid):
                        continue  # not a `cc-` tmux session we launched — leave it alone
...
async def _resume_into_existing_channel(chan, ch_id: int, session_id: str, cwd: str, user_id: int) -> bool:
    ...
    try:
        sess_name = _spawn_session_window(f"{CLAUDE_BIN} --resume {session_id}", cwd, title=session_id[:8])
    except Exception as e:
    ...
    while time.time() < deadline:
        ...
        if diff:
            ...
            break
    if not new_pid:
        print(f"  restore: no claude process appeared for {session_id[:8]}")
        return False
...
async def _adopt_orphan_live_sessions():
    ...
    for c in list_running():
        if c.pid in attached_now:
            ...
            continue
        if not _is_bot_managed(c.pid):
            continue  # only adopt `cc-` tmux sessions we launched, not stray claudes
        if not c.name:
...
async def on_ready():
    ...
    for ch_id, pid in sessions.all_attached():
        ...
        if not chan:
            sessions.set_attached_pid(ch_id, None)
            continue
        # A warm bot restart: legit terminals are still alive in their `cc-` tmux session.
        # Anything whose pid is alive but NOT in a cc- tmux session is a stray claude that
        # an older build wrongly adopted — clear it instead of re-mirroring it.
        if pid_alive(pid) and not _is_bot_managed(pid):
            sessions.set_attached_pid(ch_id, None)
            continue
        if pid in seen_pids:
            sessions.set_attached_pid(ch_id, None)
...
async def _process_message(message: discord.Message) -> None:
    ...
    if not message.content.startswith(PREFIX):
        # Bare message (no `!cc`) in an allowed, non-attached channel → run it as an
        # SDK task. This is the "just type a task from your phone" workflow; without
        # it, plain messages in a control channel are silently ignored.
        ...
        await _safe_react(message, "⌛")
        try:
            await cmd_ask(message.channel, message.channel.id, message.author.id, text)
        except Exception:
            await _safe_unreact(message, "⌛")
            await _safe_react(message, "❌")
            raise
